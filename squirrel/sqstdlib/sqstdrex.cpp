...
#ifdef SQUIRREL_SP
#define OP_GREEDY       (MAX_UCSCHAR+1) // * + ? {n}
#define OP_OR           (MAX_UCSCHAR+2)
#define OP_EXPR         (MAX_UCSCHAR+3) //parentesis ()
#define OP_NOCAPEXPR    (MAX_UCSCHAR+4) //parentesis (?:)
#define OP_DOT          (MAX_UCSCHAR+5)
#define OP_CLASS        (MAX_UCSCHAR+6)
#define OP_CCLASS       (MAX_UCSCHAR+7)
#define OP_NCLASS       (MAX_UCSCHAR+8) //negates class the [^
#define OP_RANGE        (MAX_UCSCHAR+9)
#define OP_CHAR         (MAX_UCSCHAR+10)
#define OP_EOL          (MAX_UCSCHAR+11)
#define OP_BOL          (MAX_UCSCHAR+12)
#define OP_WB           (MAX_UCSCHAR+13)
#define OP_MB           (MAX_UCSCHAR+14) //match balanced
#else
...
#ifdef SQUIRREL_SP
static SQInt32 sqstd_rex_escapechar(SQRex *exp)
#else
...
#endif
{
    if(*exp->_p == SQREX_SYMBOL_ESCAPE_CHAR){
        ...
        if(IS_SURROGATE_PAIR(*exp->_p, *(exp->_p+1))) {
            SQInt32 u = SURROGATE_PAIR_TO_UCSCHAR(*exp->_p, *(exp->_p+1));
            exp->_p+=2;
            return u;
        }
        else {
...
#ifdef SQUIRREL_SP
    else if(!scisprint(*exp->_p) && !IS_HIGH_SURROGATE(*exp->_p) && !IS_LOW_SURROGATE(*exp->_p)) {
#else
    else if(!scisprint(*exp->_p)) {
#endif
        sqstd_rex_error(exp,_SC("letter expected"));
...
#ifdef SQUIRREL_SP
    if(IS_SURROGATE_PAIR(*exp->_p, *(exp->_p+1))) {
        t = SURROGATE_PAIR_TO_UCSCHAR(*exp->_p, *(exp->_p+1));
        exp->_p+=2;
    }
    else {
        t = *exp->_p; exp->_p++;
...
static SQInteger sqstd_rex_class(SQRex *exp)
{
    ...
    while(*exp->_p != ']' && exp->_p != exp->_eol) {
        if(*exp->_p == '-' && first != -1){
            ...
            if(IS_SURROGATE_PAIR(*exp->_p, *(exp->_p+1))) {
                SQInt32 u = SURROGATE_PAIR_TO_UCSCHAR(*exp->_p, *(exp->_p+1));
                if(exp->_nodes[first].type>u) sqstd_rex_error(exp,_SC("invalid range"));
            }
            else {
...
#ifdef SQUIRREL_SP
static SQBool sqstd_rex_matchclass(SQRex* exp,SQRexNode *node,SQInt32 c)
#else
...
static const SQChar *sqstd_rex_matchnode(SQRex* exp,SQRexNode *node,const SQChar *str,SQRexNode *next)
{
    ...
    case OP_CLASS:
        if (str == exp->_eol) return NULL;
#ifdef SQUIRREL_SP
        if(IS_SURROGATE_PAIR(*str, *(str+1))) {
            SQInt32 u = SURROGATE_PAIR_TO_UCSCHAR(*str, *(str+1));
            if(sqstd_rex_matchclass(exp,&exp->_nodes[node->left],u)?(type == OP_CLASS?SQTrue:SQFalse):(type == OP_NCLASS?SQTrue:SQFalse)) {
                str+=2;
                return str;
            }
        }
...
SQRex *sqstd_rex_compile(const SQChar *pattern,const SQChar **error)
{
    ...
    if(setjmp(*((jmp_buf*)exp->_jmpbuf)) == 0) {
...
#ifdef _DEBUG
        {
            ...
            for(i = 0;i < nsize; i++) {
#ifdef SQUIRREL_SP
                if(exp->_nodes[i].type>MAX_UCSCHAR)
