...
#if defined(SQUNICODE) && defined(_MSC_VER)
static SQInteger sqstd_rex_escapechar(SQRex *exp)
#else
...
#endif
{
	if(*exp->_p == SQREX_SYMBOL_ESCAPE_CHAR){
		...
		if(IS_SURROGATE_PAIR(*exp->_p, *(exp->_p+1))) {
			SQRexNodeType u = SURROGATE_PAIR_TO_UCSCHAR(*exp->_p, *(exp->_p+1));
			exp->_p+=2;
			return u;
		}
		else {
...
#if defined(SQUNICODE) && defined(_MSC_VER)
static SQBool sqstd_rex_matchclass(SQRex* exp,SQRexNode *node,SQRexNodeType c)
#else
...
static const SQChar *sqstd_rex_matchnode(SQRex* exp,SQRexNode *node,const SQChar *str,SQRexNode *next)
{
	...
	case OP_CLASS:
		...
		if(IS_SURROGATE_PAIR(*str, *(str+1))) {
			SQRexNodeType u = SURROGATE_PAIR_TO_UCSCHAR(*str, *(str+1));
